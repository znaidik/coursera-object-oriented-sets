...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted: Tweet = {
      var mr: Tweet = elem
      this.foreach(tw => if(tw.retweets > mr.retweets) mr = tw)
      mr
    }
...
object GoogleVsApple {
  ...
  val apple = List("ios", "iOS", "iphone", "iPhone", "ipad", "iPad")
    ...
    lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(x => google.exists(tw => x.text.contains(tw)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(x => apple.exists(tw => x.text.contains(tw)))
